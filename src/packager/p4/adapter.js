...
class P4Adapter extends WebAdapter {
  async fetchExtensionScript (url, options = {}) {
    ...
    if (environment === 'standalone') {
      ...
      try {
        return await super.fetchExtensionScript(url, options);
      } catch (error) {
        ...
        return options.wrap === false ? source : `(function(Scratch) { ${source} })(Scratch);`;
      }
    }
    
    // 浏览器环境直接使用父类方法
    return super.fetchExtensionScript(url, options);
  }
}
